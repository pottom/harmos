...
| `e` · `n` / `N` · `d` / `D` · `m` | Edit an entry, or rename a folder on its own row · new entry / folder · delete to bin / permanently (press again to undo) · move |
...
2. **Change** things: `e` edits, `n` and `N` create an entry or a folder, `d`
   sends to the recycle bin (`D` deletes permanently), `m` moves. There is one
   editing key: an entry has fields, so `e` opens the form; a folder is its
   name, so `e` makes the name editable on its own row, with the tree still on
   screen behind it. `ctrl+g` rolls a password in the editor using your
