...
| `e` · `r` · `n` / `N` · `d` / `D` · `m` | Edit · rename in place · new entry / folder · delete to bin / permanently (press again to undo) · move |
...
2. **Change** things: `e` edits, `n` and `N` create an entry or a folder, `d`
   ...
   screen. `ctrl+g` rolls a password in the editor using your Generate-tab
   settings.
