...
|---|---|---|
| ~~**0**~~ | ~~`docs/m6-write-model`~~ · **done** | This document exists; the workflow and spec name M6 and its limits. Zero code change. |
...
| **2** | `feat/vault-mutations` (in review) | create/edit/move/delete round-trip on a temp kdbx; `keepassxc-cli show` reads the new entry, the bin group holds the binned one, and a permanently deleted UUID appears in `DeletedObjects`. |
