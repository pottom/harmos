...
| **M3** | **Local kdbx side** — read external `.kdbx` files as sources. | An external file is browsable read-only and its bytes are provably unchanged after a session. *(M6 later made writing possible, behind an explicit per-source unlock, remembered in the config; browsing still changes nothing.)* |
