...
//
// Reading is strictly read-only: Open takes O_RDONLY and writes nothing, so
// browsing a user's own kdbx leaves its bytes and mtime unchanged. That is still
// an invariant and still tested.
//
// Writing exists, and it is deliberately narrow (see handle.go and write.go):
// only a Handle can write, only its Save method does so, and only after the
// caller has been through the refusals in refuseWriteBecause. Pleasant caches
// never get a Handle — they are regenerated by sync, so an edit there would be
...
//
// Entry is a PROJECTION, and a lossy one: it drops the kdbx UUIDs, the group
// attributes, history, custom data, icons and auto-type, and it rewrites values
// on the way out (oneline collapses control characters, customLabel strips the
...
// source. It opens the file read-only and writes nothing. creds must carry a
// password and/or keyfile.
//
// This is the browsing path: it drops the handle immediately, so nothing here
// can write. A caller that intends to save uses OpenHandle and takes its
// Snapshot — one decode path, two entry points.
func Open(path, source string, creds Credentials) (*Vault, error) {
	h, err := OpenHandle(path, source, creds)
	if err != nil {
		return nil, err
	}
