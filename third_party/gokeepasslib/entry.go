...
type Entry struct {
	...
	// QualityCheck is KDBX 4.1. A pointer so that "absent" stays distinct from
	// "present and false": the element defaults to true when missing, so writing
	// an explicit value into a file that never had one would change its meaning.
	QualityCheck *w.BoolWrapper `xml:"QualityCheck,omitempty"`
	// PreviousParentGroup is KDBX 4.1: the UUID of the group this entry was in
...
func (e *Entry) setKdbxFormatVersion(version formatVersion) {
	...
	for i := range e.CustomData {
		(&e.CustomData[i]).setKdbxFormatVersion(version)
	}
...
type CustomData struct {
	...
	// LastModificationTime is KDBX 4.1, and lets a synchronising client tell
	// which side of a merge holds the newer value.
